...
local function getSH()
    if not _SH then
        ...
        else logger.warn("simpleui: module_collections: cannot load module_books_shared: " .. tostring(m)) end
    end
    return _SH
end
...
local function getDims(scale, thumb_scale)
    ...
    local shadow_off   = math.max(1, Screen:scaleBySize(2))
    ...
    return {
        ...
        stack_cell_w = coll_w + stack_extra + shadow_off,
        cell_h       = coll_h + shadow_off + accent_h,
        coll_cell_h  = coll_h + shadow_off + accent_h + label_gap + 2 * tbw_line_h,
...
local function buildCoverCell(files, cover_override, coll_name, count, d)
    ...
    local cover_frame
    local cover_radius = Screen:scaleBySize(6)
    ...
    local cover = cover_frame
    local SH = getSH()
    if SH and SH.wrapWithShadow then
        cover = SH.wrapWithShadow(cover_frame, d.coll_w, d.coll_h) or cover_frame
    end
    ...
    local accent = FrameContainer:new{
        ...
        dimen      = Geom:new{ w = d.coll_w + d.shadow_off, h = d.accent_h },
...
function M.build(w, ctx)
    ...
    if lbl_scale ~= 1.0 then
        ...
        d.coll_cell_h = d.coll_h + d.shadow_off + d.accent_h + d.label_gap + 2 * d.tbw_line_h
    end
    ...
    for _, slot in ipairs(widget._cover_slots) do
        ...
        if raw then
            ...
            if ok and img then
                ...
                if SH and SH.wrapWithShadow then
                    slot.container[slot.idx] = SH.wrapWithShadow(cover_frame, slot.w, slot.h) or cover_frame
                else
                    slot.container[slot.idx] = cover_frame
                end
            end
...
function M.getHeight(ctx)
    local d = getDims(Config.getModuleScale("collections", ctx and ctx.pfx),
                      Config.getThumbScale("collections", ctx and ctx.pfx))
