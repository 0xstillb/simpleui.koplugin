...
function M.build(w, ctx)
    ...
    local cell_h  = (use_overlay and (ch + badge_r) or D.RECENT_CELL_H) + shadow_extra
    ...
    for i = 1, cols do
        ...
        local raw_cover = SH.getBookCover(fp, cw, ch, nil, 0.10) or SH.coverPlaceholder(bd.title, bd.authors, cw, ch)
        ...
        if use_overlay then
            local shadow_group = cover_widget[1]
            if shadow_group and shadow_group[2] then
                cover_slots[#cover_slots+1] = { container = shadow_group, idx = 2, fp = fp, w = cw, h = ch, align = nil, stretch = 0.10 }
            else
...
function M.getHeight(_ctx)
    ...
    local shadow_extra = math.max(1, Screen:scaleBySize(2))
    local h = D.RECENT_H + shadow_extra
    if use_overlay then
