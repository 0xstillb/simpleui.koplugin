...
function M.build(w, ctx)
    ...
    -- B2 overlap stack: side covers partially overlap the center cover,
    -- creating a stacked-cards effect with depth through border darkness.
    local offset_near = math.floor(center_w * 0.22)  -- sides peek out ~22% from center edge
    local offset_far  = math.floor(center_w * 0.42)  -- far covers overlap sides
    local TOP_CLEAR   = 2
    ...
    -- Covers — wrapped in pcall so a single corrupt book never crashes the deck.
    -- `add_shadow`: when true, wraps the cover with a drop shadow for depth.
    --              Only used on the centre cover to avoid clutter on side/far covers.
    local function buildCover(fp, cw, ch, align, add_shadow)
        local ok, cover = pcall(function()
            local bd = SH.getBookData(fp, ctx.prefetched and ctx.prefetched[fp])
            local c = SH.getBookCover(fp, cw, ch, align, 0.20)
                      or SH.coverPlaceholder(bd.title, bd.authors, cw, ch)
            if add_shadow then
                return SH.wrapWithShadow(c, cw, ch) or c
            end
            return c
        end)
        if ok and cover then return cover end
        -- Fallback: minimal gray rectangle so the carousel still renders.
        logger.warn("coverdeck: buildCover failed for " .. tostring(fp) .. ": " .. tostring(cover))
        return FrameContainer:new{
            width = cw, height = ch,
            bordersize = Screen:scaleBySize(1),
            radius     = Screen:scaleBySize(6),
            padding = 0, margin = 0,
            background = require("ffi/blitbuffer").gray(0.88),
    ...
    items[#items+1] = buildCover(fps[curIdx], center_w, center_h, "center", true)
    ...
    -- Book data for centre cover — pcall-guarded so corrupt metadata
    -- never crashes the carousel.
    ...
    local bd
    local ok_bd = pcall(function()
        bd = SH.getBookData(fps[curIdx], ctx.prefetched and ctx.prefetched[fps[curIdx]])
    end)
    if not ok_bd or not bd then
        bd = { title = "?", authors = "", percent = 0 }
    end
    -- Thai-aware font faces for centre cover title/info
    local title_text = truncateTitle(bd.title, 30)
    local face_title, face_info
    local ok_face = pcall(function()
        face_title = SH.getFaceForText(title_text, math.max(8, title_fs), "smallinfofont")
        face_info  = Font:getFace("smallinfofont", math.max(7, info_fs))
    end)
    if not ok_face or not face_title then
        face_title = Font:getFace("smallinfofont", math.max(8, title_fs))
        face_info  = Font:getFace("smallinfofont", math.max(7, info_fs))
    end
    ...
    if show_progress then
        meta[#meta+1] = SH.progressBar(center_w, bd.percent, bar_h, true)
    end
